...
import com.hinmu.lims.util.common.PropertiesUtil;
...
public class JwtFilter extends BasicHttpAuthenticationFilter {
    ...
    private boolean refreshToken(ServletRequest request, ServletResponse response) {
        ...
        if (JedisUtil.exists(Constant.getPREFIX_SHIRO_REFRESH_TOKEN(LoginClientTypeEnum.valueOf(loginClientType))+ account)) {
            ...
            if (JwtUtil.getClaim(token, Constant.CURRENT_TIME_MILLIS).equals(currentTimeMillisRedis)) {
                ...
                JedisUtil.setObject(Constant.getPREFIX_SHIRO_REFRESH_TOKEN(LoginClientTypeEnum.valueOf(loginClientType)) + account, currentTimeMillis, Integer.parseInt(refreshTokenExpireTime));
                // 刷新AccessToken，设置时间戳为当前最新时间戳
                token = JwtUtil.sign(userid,account,loginClientType, currentTimeMillis);
    ...
    private void response401(ServletResponse response, String msg) {
        HttpServletResponse httpServletResponse = WebUtils.toHttp(response);
        //todo 此处还是将http状态码修改成 200通过错误码来展示
        //httpServletResponse.setStatus(HttpStatus.UNAUTHORIZED.value());
        httpServletResponse.setStatus(HttpStatus.OK.value());
        httpServletResponse.setCharacterEncoding("UTF-8");
        httpServletResponse.setContentType("application/json; charset=utf-8");
        try (PrintWriter out = httpServletResponse.getWriter()) {
            String data = JsonConvertUtil.objectToJson(new RespResultMessage().unauthorized401());
            out.append(data);
        } catch (IOException e) {
            logger.error("直接返回Response信息出现IOException异常:" + e.getMessage());
            throw new ApiClientException("直接返回Response信息出现IOException异常:" + e.getMessage());
        }
    }
}
