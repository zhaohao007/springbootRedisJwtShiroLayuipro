...
;layui.define(["laytpl", "laypage", "layer", "form", "util"], function (e) {
    "use strict";
    var t = layui.$, i = layui.laytpl, a = layui.laypage, l = layui.layer, n = layui.form,setter = layui.setter,responseFromSetter = setter.response,admin = layui.admin,
    ...
    F.prototype.config = {
        ...
        if (i.startTime = (new Date).getTime(), a.url) {
            ...
            a.contentType && 0 == a.contentType.indexOf("application/json") && (d = JSON.stringify(d)), t.ajax({
                ...
                dataType: "json",
                headers: a.headers || {},
                success: function (t,textStatus,request) {
                    /** todo 修改源代码以适配服务端接口 */
                    switch (t.code) {
                        case 200:
                            var headerAuthorization = request.getResponseHeader(responseFromSetter.headerAuthorization);
                            alert("是否存在刷新token："+headerAuthorization);
                            alert("table返回数据结果:"+JSON.stringify(t));
                            if(headerAuthorization!='' && headerAuthorization!='null' && headerAuthorization!=undefined){
