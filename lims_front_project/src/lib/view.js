...
layui.define(['laytpl', 'layer'], function(exports){
  ...
  view.req = function(options){
    ...
    return $.ajax($.extend({
      type: 'get'
      ,dataType: 'json'
      ,success: function(res,textStatus,request){//todo 需要获取response中的header值
        var statusCode = response.statusCode;
        var headerAuthorization;
        //只有 response 的 code 一切正常才执行 done
        if(res[response.statusName] == statusCode.ok) {
          typeof options.done === 'function' && options.done(res);
          headerAuthorization = request.getResponseHeader(response.headerAuthorization);
        ...
        typeof success === 'function' && success(res,headerAuthorization);
      }
